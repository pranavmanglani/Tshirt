...
def add_to_cart(product_id, quantity):
    ...
    st.rerun() # Rerunning is still necessary for instant UI update
...
def customer_checkout(cart_df):
    ...
    if st.session_state.get('checkout_substage') != 'delivered':
        ...
        for index, row in cart_df.iterrows():
            col_index = index % 3
            cols_remove[col_index].button(f"Remove 1x {row['name']} ({row['size']})", key=f"remove_{row['cart_item_id']}", 
...
def customer_browse_products():
    ...
    # Optimization: Use cached data to avoid re-querying the DB every time the script runs (unless a new product was added)
    ...
    # Apply filter based on search query (this filtering happens in memory, which is fast)
    if search_query:
    ...
    # Display Products
    cols = st.columns(min(3, len(products_df))) 
...
def product_detail_view():
    ...
    conn = get_db_connection()
    # Optimized query: Fetch single product data directly
    c = conn.cursor()
    ...
    with col_info:
        ...
        # In a real app, this would be a selection of available variants (size/color)
        # Optimization: Ensure the current product size is an option
        available_sizes = sorted(list(set([product['size'], "S", "M", "L", "XL"])))
        selected_size = st.selectbox("Select Size", available_sizes, index=available_sizes.index(product['size']))
        ...
        if st.button(f"Add {quantity}x to Cart", type="primary", key="add_to_cart_detail"):
            # NOTE: We are adding the original product ID (product['id']) to the cart,
            # even if the user selected a different size, as our current database
            # structure doesn't support multiple sizes/colors per single product ID efficiently.
            # In a real system, the product_id would likely be a SKU/Variant ID combining size/color.
